...
export function badWithBody(body: {}, err: Err = 400): never {
  if (err === 500) throw new InternalServerErrorException(body);
  if (err === 401) throw new UnauthorizedException(body);
  else throw new BadRequestException(body);
}
